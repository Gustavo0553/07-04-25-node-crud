        ...
        switch (tipo) {
            case 'cadastro':
                var [rows, fields] = await pool.query(
                    "insert into `gustavo_crud`.`tbl_login` (`nome`, `login`, `senha`) values (?, ?, ?);",
                    [nome, login, senha]
                );
            ...
            case 'login':
                var [rows, fields] = await pool.query(
                    "select * from `gustavo_crud`.`tbl_login` where `nome` = ? and `login` = ? and `senha` = ?;",
                    [nome, login, senha]
                );
                if (rows.length == 1) {
            ...
            case 'leitura':
                ...
                var strSql = "select * from `gustavo_crud`.`tbl_login` where" + 
                ...
                break;
            case 'atualizacao':
                var strSql = "select * from `gustavo_crud`.`tbl_login`;";
                var [rows, fields] = await pool.query(strSql);
                if (rows.length > 0) {
                    res.json({ 
            ...
            case 'atualizar':
                ...
                var strSql = "update `gustavo_crud`.`tbl_login` set " + 
